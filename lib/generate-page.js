...
export async function generatePageVariables(pageId) {
  ...
  const [sections] = await pool.query(
    "SELECT s.id, s.variables, s.variable_origins, st.variables as type_variables, st.default_content FROM sections s JOIN section_types st ON s.section_type_id = st.id WHERE s.page_id = ? ORDER BY s.sort_order",
    [pageId]
  );
  ...
  for (const { sec, vars, origins, contextUpdates, contentToGenerate } of plans) {
    ...
    if (contentToGenerate.length) {
      try {
        const result = await callLLM({ prompt: buildGenPrompt(ctx, contentToGenerate), objectType: "variable_generation", objectKey: `page:${pageId}` });
