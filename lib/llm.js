 ...
 */
export async function callLLM({ provider = "gemini", prompt, currentHtml, objectType, objectKey, imageData }) {
  const apiKey = await getKey(KEY_MAP[provider] || KEY_MAP.gemini);
  if (!apiKey) throw new Error(`No API key configured for ${provider}`);

  // Build prompt chain. Variable generation must return JSON, so skip the global
  // "system" prompt (which mandates HTML output) for that task.
  const sysRow = objectType === "variable_generation" ? null : await getActivePromptRow("system");
  ...
  const systemPrompt = [sysRow?.content, typeRow?.content, objRow?.content].filter(Boolean).join("\n\n");
