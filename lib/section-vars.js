...
export function buildGenPrompt(ctx, contentToGenerate) {
  ...
  return `IMPORTANT: This is a variable generation task, NOT an HTML generation task. Ignore any instructions about generating HTML. Return ONLY a raw JSON object with string values, no HTML, no markdown fences, no explanation.\n\n${contextLine}\n\nGenerate concise, ready-to-publish text values for these variables. Use the variable name as a guide to what each should contain:\n${contentToGenerate.map(v => `- ${v.key}: ${v.hint}`).join("\n")}`;
}
