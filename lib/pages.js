...
export async function getPageBySlug(slug, publishedOnly = true) {
  ...
  page.sections = page.sections.map(s => {
    ...
    return { ...s, content: substituteVars(substituteVars(s.content, vars), config, strip) };
  });
  ...
  page.body_content = substituteVars(injectContent(tpl, sectionsHtml), config, strip);
  return page;
}
...
// Public HTML lang. Traditional Chinese by default; override via `site_lang`.
// Resilient: returns the default if the DB is unavailable (e.g. at build time).
export async function getSiteLang() {
  try {
    const config = await getSiteConfig();
    return config.site_lang || DEFAULT_LANG;
  } catch {
    return DEFAULT_LANG;
  }
}

// Given the catch-all slug segments and the resolved content path, return the
// page slug (content_path prefix stripped), or null if the URL does not match
// the configured prefix. Shared by the page renderer and generateMetadata.
export function resolvePageSlug(slugSegments, contentPath) {
  const joined = "/" + slugSegments.join("/");
  if (contentPath) {
    if (!joined.startsWith(contentPath + "/")) return null;
    return joined.slice(contentPath.length + 1) || null;
  }
  return slugSegments.join("/") || null;
}
